...
export interface StorageDetailDto extends StorageStatsDto {
  eventCount: number;
  videoCount: number;
}
...
export interface SearchEventsResultDto {
...
export interface StatisticsParams {
  from: string;
  to: string;
  groupBy?: 'hour' | 'day' | 'kind';
}

export interface StatisticsGroupDto {
  key: string;
  count: number;
  persons: number;
  animals: number;
...
export interface ConfigurationDto {
  ...
  notifications: {
    enabled: boolean;
    detectionNotifications: boolean;
  };
}
...
export interface MediaResourceContent {
  mimeType: string;
  data: Buffer | string; // binary buffer or string content
}
