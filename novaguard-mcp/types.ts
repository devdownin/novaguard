...
export interface DetectionEventDto {
  id: number; kind: DetectionKind; timestamp: string; durationSeconds: number; confidence: number;
  hasVideo: boolean; videoResourceUri: string | null; thumbnailResourceUri: string | null; sizeBytes: number;
}
export interface StorageStatsDto { usedBytes: number; freeBytes: number; totalBytes: number; }
export interface StatusDto { surveillanceActive: boolean; camera: string; lastDetectionAt: string | null; detectionsToday: number; storage: StorageStatsDto; }
export interface StorageDetailDto extends StorageStatsDto { eventCount: number; videoCount: number; }
export interface SearchEventsParams { from?: string; to?: string; kind?: DetectionKind; minConfidence?: number; hasVideo?: boolean; limit?: number; offset?: number; sort?: 'timestamp_desc' | 'timestamp_asc'; }
...
export interface StatisticsResultDto { period: { from: string; to: string }; total: number; persons: number; animals: number; averageConfidence: number; withVideo: number; withoutVideo: number; groups?: StatisticsGroupDto[]; }
