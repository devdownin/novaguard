...
export class NovaGuardMcpServer {
  ...
  public getCapabilities() {
    return {
      ...
      serverInfo: { name: 'novaguard-mcp', version: '1.0.0' },
      protocolVersion: MCP_PROTOCOL_VERSION,
    };
  }

  private validateJsonRpcRequest(req: any): asserts req is JsonRpcRequest {
    if (!req || typeof req !== 'object' || Array.isArray(req) || req.jsonrpc !== '2.0' || typeof req.method !== 'string' || req.method.length === 0 || req.method.length > 128) {
      throw new McpError('NOVAGUARD_INVALID_REQUEST', 'Invalid JSON-RPC request', 400);
    }
    if (Object.prototype.hasOwnProperty.call(req, 'id') && req.id !== null && typeof req.id !== 'string' && typeof req.id !== 'number') {
      throw new McpError('NOVAGUARD_INVALID_REQUEST', 'Invalid JSON-RPC id', 400);
    }
    if (req.params !== undefined && (!req.params || typeof req.params !== 'object' || Array.isArray(req.params))) {
      throw new McpError('NOVAGUARD_INVALID_REQUEST', 'JSON-RPC params must be an object', 400);
    }
  }

  private validateInitialize(params: any): void {
    if (!params || typeof params !== 'object' || Array.isArray(params)) {
      throw new McpError('NOVAGUARD_INVALID_REQUEST', 'initialize params are required', 400);
    }
    if (params.protocolVersion !== MCP_PROTOCOL_VERSION) {
      throw new McpError('NOVAGUARD_INVALID_REQUEST', `Unsupported MCP protocol version. Expected ${MCP_PROTOCOL_VERSION}`, 400);
    }
    if (!params.capabilities || typeof params.capabilities !== 'object' || Array.isArray(params.capabilities)) {
      throw new McpError('NOVAGUARD_INVALID_REQUEST', 'initialize capabilities are required', 400);
    }
    if (!params.clientInfo || typeof params.clientInfo !== 'object' || typeof params.clientInfo.name !== 'string' || typeof params.clientInfo.version !== 'string') {
      throw new McpError('NOVAGUARD_INVALID_REQUEST', 'initialize clientInfo is required', 400);
    }
  }
    ...
    try {
      ...
      switch (req.method) {
        ...
        case 'tools/call': {
          ...
          if (typeof toolName !== 'string' || toolName.length === 0 || toolName.length > 128) {
            throw new McpError('NOVAGUARD_INVALID_ARGUMENT', 'Tool name is required', 400);
          }
          const toolArgs = req.params?.arguments ?? {};
          if (!toolArgs || typeof toolArgs !== 'object' || Array.isArray(toolArgs)) {
            throw new McpError('NOVAGUARD_INVALID_ARGUMENT', 'Tool arguments must be an object', 400);
          }
          operation = `tool:${toolName}`;
          if (typeof toolArgs.eventId === 'number') eventId = toolArgs.eventId;
        ...
        case 'resources/read': {
          ...
          if (res.blob) {
            ...
            if (mediaBytes > maxBytes) throw new McpError('NOVAGUARD_MEDIA_UNAVAILABLE', 'Media exceeds configured maximum size', 413);
            result = { contents: [{ uri: res.uri, mimeType: res.mimeType, blob: res.blob.toString('base64') }] };
          } else {
            result = { contents: [{ uri: res.uri, mimeType: res.mimeType, text: res.text }] };
          }
          break;
        }
        default:
      ...
      return {
        ...
        id: requestId,
        error: {
          code: errorCode === 'NOVAGUARD_INVALID_REQUEST' ? -32600 : statusCode === 404 ? -32601 : statusCode === 400 ? -32602 : -32603,
